...
class SymNSQ(MemoryBankModule):
    ...
    Examples:
        ...
        >>> loss_fn = SymNSQ(memory_bank_size=0)
        >>>
    ...
    def forward(self, out0: torch.Tensor, out1: torch.Tensor):
        ...
        out1, negatives = super(SymNSQ, self).forward(
        ...
        affinity_map = create_adjacency_matrix(labels).to(device)

        # logits = normalize_with_diagonal_zero(logits**2)
        # logits_aff_sum = (logits/2 + affinity_map/2).log()
        # # loss =  self.cross_entropy(logits_aff_sum, logits) + self.cross_entropy(logits_aff_sum, affinity_map)
        # loss = F.kl_div(logits_aff_sum, affinity_map) + F.kl_div(logits_aff_sum, logits)

        epsilon = 1e-10

        # Assuming logits and affinity_map are already probabilities
        logits = normalize_with_diagonal_zero(logits**2)
        logits = torch.clamp(logits, min=epsilon, max=1)  # Ensure logits are within [0,1]
        affinity_map = torch.clamp(affinity_map, min=epsilon, max=1)  # Ensure affinity_map is within [0,1]

        # loss = F.kl_div(logits.log(), affinity_map, reduction='batchmean')
        logits_aff_sum = (logits / 2 + affinity_map / 2).log()  # Log space
        #
        # Calculate the loss using F.kl_div
        loss = F.kl_div(logits_aff_sum, affinity_map, reduction='batchmean') + \
               F.kl_div(logits_aff_sum, logits, reduction='batchmean')
