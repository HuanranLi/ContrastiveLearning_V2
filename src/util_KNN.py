...
def self_test_knn_accuracy(feature_bank, feature_labels, classes, knn_k=1, knn_t=0.07):
    """Tests the kNN function on the feature bank itself to ensure 100% accuracy with k=1."""

    # Run kNN prediction on the feature bank
    predicted_labels = knn_predict(feature_bank, feature_bank, feature_labels, classes, knn_k, knn_t)

    # Check if predicted labels match the actual labels
    correct_predictions = (predicted_labels == feature_labels).sum().item()
    total_predictions = feature_labels.size(0)
